...
const Models = () => {
  ...
  // Buscar la variante "ClassicOak" en cada producto
  const models = Products.map((product) => {
    const classicOakVariant = product.variants.find((variant) => variant.name.includes('ClassicOak'));

    return {
      image: classicOakVariant ? classicOakVariant.image : product.defaultImage, // Usar imagen por defecto si no hay ClassicOak
      name: product.name,
    };
  });
